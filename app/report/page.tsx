...
import { Brain, Loader2, Send, FileText, Calendar, ChevronRight } from 'lucide-react';
...
export default function Report() {
  ...
  const ask = async () => {
    ...
    try {
      const res = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question, mode: 'deep' }) });
      const d = await res.json();
      if (d.data) setAnalysis(d.data);
    ...
    <div style={{ minHeight: '100vh', background: T.bg, color: T.text, fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <main style={{ maxWidth: 800, margin: '0 auto', padding: '36px 28px 80px' }}>
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: T.sub }}>Claude Opus with your shared context and latest metrics. Specific dollar math, no generic advice.</p>
        </motion.div>
          ...
          <>
            ...
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: '16px 18px', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10, paddingTop: 10, borderTop: `1px solid ${T.border}` }}>
                <span style={{ fontSize: 10, color: T.dim, fontFamily: 'DM Mono, monospace' }}>POWERED BY CLAUDE OPUS</span>
        ...
        {mode === 'weekly' && (
          <div>
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: '24px', marginBottom: 20, textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 13, color: T.sub, marginBottom: 20, lineHeight: 1.6 }}>Claude Opus compares your current metrics to the previous period, identifies trends, and generates a full intelligence brief with prioritized actions.</p>
